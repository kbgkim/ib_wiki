...
본 프로젝트는 투자은행(IB) 실무에서 다루는 4대 핵심 자산군(**NPL**, **PF**, **Equity**, **ABS**)의 지식을 체계적으로 구조화하고, 이를 '미래 현금흐름의 불확실성'이라는 단일 관점으로 통합하여 관리하는 프로덕션급 지식 위키입니다.
...
- 🚀 **[통합 리스크 대시보드](src/DASHBOARD.md)**: 4대 자산 통합 관제 및 리스크 현황
- 📖 **[01_Foundations](src/01_Foundations/IB_Overview.md)**: IB 기본 개념, 가치 사슬, 전사 리스크 정책
- 🧠 **[02_Integrated_IB](src/02_Integrated_IB/Synthesis_Map.md)**: 자산군 간 시너지 맵, 통합 리스크 프레임워크 및 기술 사양
- 📊 **[03_Assets_Verticals](src/03_Assets_Verticals/NPL/Basics.md)**: NPL, PF, ABS, Equity 각 도메인의 심층 기초 지식
- 📥 **[00_Inbox](src/00_Inbox/README.md)**: 신규 지식 입수 및 자동 분류 대기소
- ⚙️ **[99_System](src/99_System/PROJECT_CONTEXT.md)**: 위키 운영 계획, 로깅 및 프로젝트 문맥 정보
...
- 모든 문서 하단에는 **'관련 문서'** 링크가 구성되어 상호 참조가 가능합니다.

### 2. 신규 문서 입수 (AI Workflow)
기존에 정리된 파일이나 새로운 리서치 자료가 있다면 다음 단계를 따르세요:
- 파일을 `src/00_Inbox/` 폴더에 업로드합니다.
- Antigravity(AI 에이전트)에게 **"인박스 처리해줘"**라고 요청합니다.
- AI가 문서를 분석하여 위키 표준으로 변환한 후, 적절한 카테고리로 자동 이동 및 아카이브합니다.
